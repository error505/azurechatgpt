...
import { FindAllChatThreadsForReportingByUser } from "./reporting-service";
...
export const Reporting = async (props: ReportingProp) => {
  ...
  const { resources: chatThreads } = await FindAllChatThreadsForReportingByUser(
    pageSize,
    pageNumber
  );
                  ...
                  chatThreads.map((chatThread) => (
                    <TableRow key={chatThread.id}>
                      <TableCell className="font-medium">
                        <Link href={"/reportingbyuser/" + chatThread.id}>
                          {chatThread.id}
                        </Link>
                      </TableCell>
