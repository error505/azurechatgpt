...
export async function writeBlob(data: any) {
  ...
  // download file
  return await blobClient.upload(
    JSON.stringify(data),
...
export async function getLogoData(fileName: string) {
  ...
  if (!(await blobClient.exists())) {
    const fileContents = await fs.readFile("public/4583356.png", "base64");
    return fileContents;
  }
...
export async function uploadLogo(formData: FormData) {
  ...
  const formDataArrayBuffer = await (
